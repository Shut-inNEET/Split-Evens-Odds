...
void intLinkedList::splitEvensOddsList(intLinkedList& evensList, intLinkedList& oddsList)
{
    ...
    while (current != nullptr) //while current hasnt reached the end of the list
    {
        if (current->info % 2 == 0) //if node contains even number
        {
            if (evensList.count == 0) //if evenslList is empty
            {
                evensList.first = current; //set first in evensList to current
            }
            else
            {
                evensList.last->link = current; //set last in evensList to point to current
            }
            evensList.last = current; //set last in evensList to current
            evensList.count++; //increment evensList count
            current = current->link; //make current point to the next node in list
            evensList.last->link = nullptr; //set last in evensList to point to nullptr 
        }
        else //apply same steps from evenslist to oddslist
        {
            if (oddsList.count == 0)
            {
    ...
    //clean up so list is an empty list
    first = nullptr; 
    last = nullptr;
    count = 0; 
}
